...
export default function Home() {
  ...
  return (
    <>
      ...
      {!online && <div className="offline-banner">⚠️ Vous etes hors-ligne. Connectez-vous au Wi-Fi local de la bibliotheque pour continuer.</div>}
      <section className="hero">
        ...
        <motion.div className="hero-content" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22,1,0.36,1] }}>
          <span className="hero-badge">🎓 Mini-projet ESPA — 2eme annee</span>
      ...
      <section className="section" id="comment-ca-marche">
        ...
        <div className="steps-grid">
          {[{ icon: '📱', title: '1. Scannez le QR Code', text: "Chaque livre a une etiquette QR unique." },{ icon: '📖', title: '2. Consultez la fiche', text: "Titre, auteur, disponibilite instantanement." },{ icon: '🔐', title: '3. Connectez-vous', text: "Creez votre compte en 1 minute." },{ icon: '📩', title: "4. Demandez l'emprunt", text: "L'administrateur valide ou refuse avec motif." }].map((s, i) => (
            <FadeInSection key={i} delay={i*0.1}><div className="step-card"><div className="step-icon">{s.icon}</div><h3>{s.title}</h3><p>{s.text}</p></div></FadeInSection>
          ))}
        </div>
      </section>
      ...
      <section className="section trust-section" id="a-propos">
        ...
        <div className="trust-grid">
          <FadeInSection delay={0.05}><div className="trust-card"><div className="trust-icon">🔒</div><h3>Donnees securisees</h3><p>Bcrypt, JWT, cookies HttpOnly. Vos donnees ne sont jamais exposees.</p></div></FadeInSection>
          <FadeInSection delay={0.1}><div className="trust-card"><div className="trust-icon">🎓</div><h3>Projet academique transparent</h3><p>Developpe par des etudiants ESPA dans un cadre pedagogique supervise.</p></div></FadeInSection>
          <FadeInSection delay={0.15}><div className="trust-card"><div className="trust-icon">📶</div><h3>Fonctionne hors-ligne</h3><p>Un serveur de secours a la bibliotheque prend le relais en cas de coupure.</p></div></FadeInSection>
          <FadeInSection delay={0.2}><div className="trust-card"><div className="trust-icon">✅</div><h3>Controle humain</h3><p>Chaque emprunt est valide manuellement par un administrateur.</p></div></FadeInSection>
        </div>
      </section>
      <Footer />
    </>
