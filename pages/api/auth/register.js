...
export default async function handler(req, res) {
  ...
  try {
    ...
    const isEtudiant = is_etudiant !== false;
    if (isEtudiant && (!etablissement || !etablissement.trim())) return res.status(400).json({ error: "L'etablissement est obligatoire pour un etudiant." });
    ...
    const finalEtablissement = isEtudiant ? (etablissement || null) : null;
    const finalNiveau = isEtudiant ? (niveau || null) : null;
    const r = await sql`INSERT INTO users (nom, prenom, numero, email, etablissement, niveau, is_etudiant, password_hash) VALUES (${nom.trim()}, ${prenom.trim()}, ${numero.trim()}, ${email.toLowerCase().trim()}, ${finalEtablissement}, ${finalNiveau}, ${isEtudiant}, ${hash}) RETURNING id, nom, prenom, email`;
    const user = r[0];
