...
export function setUserCookie(res, token) {
  res.setHeader('Set-Cookie', serialize(COOKIE_NAME, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 2592000 }));
}
export function setAdminCookie(res, token) {
  res.setHeader('Set-Cookie', serialize(ADMIN_COOKIE_NAME, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 43200 }));
}
...
export function getUserFromReq(req) {
  const c = parse(req.headers.cookie || ''); const t = c[COOKIE_NAME]; if (!t) return null; return verifyToken(t);
}
export function getAdminFromReq(req) {
  const c = parse(req.headers.cookie || ''); const t = c[ADMIN_COOKIE_NAME]; if (!t) return null;
  const d = verifyToken(t); if (!d || d.role !== 'admin') return null; return d;
}
